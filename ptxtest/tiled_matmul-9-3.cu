...
*/
	...
	for (int i = 0; i < 8; ++i) {
		...
		((float4*)&C[(row+i   )*n+bx])[id%16] = ((float4*)sA_bf[0])[baseSh];    // row  0 and  8 | 32 and 40
		((float4*)&C[(row+i+16)*n+bx])[id%16] = ((float4*)sA_bf[0])[baseSh+32]; // row 16 and 24 | 48 and 56
	}
/*
	int row = by + (id<32? 0:32);
	int idwarp = id % 32;
	int flag = id<32? 0:1;
	flag *= 256;
	for (int i = 0; i < 8; ++i) {
		...
		//0
		C[(row+i   )*n+bx+idwarp   ] = sA_bf[0][ flag     + idwarp];
		C[(row+i   )*n+bx+idwarp+32] = sA_bf[0][ flag+32  + idwarp];

		//8
		C[(row+i+ 8)*n+bx+idwarp   ] = sA_bf[0][ flag+64  + idwarp];
		C[(row+i+ 8)*n+bx+idwarp+32] = sA_bf[0][ flag+96  + idwarp];

		//16
		C[(row+i+16)*n+bx+idwarp   ] = sA_bf[0][ flag+128 + idwarp];
		C[(row+i+16)*n+bx+idwarp+32] = sA_bf[0][ flag+160 + idwarp];

		//24
		C[(row+i+24)*n+bx+idwarp   ] = sA_bf[0][ flag+192 + idwarp];
		C[(row+i+24)*n+bx+idwarp+32] = sA_bf[0][ flag+224 + idwarp];
	}
*/
}
