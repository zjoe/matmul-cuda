...
__global__ void matMul(const float *A, const float *B, float *C, int m, int k, int n)
{
	...
	for (int t = 0; t < k; t += 8) {
		sA[0][y8x] = tex2D(tex_A, t  , idA_global);
		sA[1][y8x] = tex2D(tex_A, t+1, idA_global);
		sA[2][y8x] = tex2D(tex_A, t+2, idA_global);
		sA[3][y8x] = tex2D(tex_A, t+3, idA_global);
		sA[4][y8x] = tex2D(tex_A, t+4, idA_global);
		sA[5][y8x] = tex2D(tex_A, t+5, idA_global);
		sA[6][y8x] = tex2D(tex_A, t+6, idA_global);
		sA[7][y8x] = tex2D(tex_A, t+7, idA_global);

		sB[0][y8x] = tex2D(tex_B, idB_global, t  );
		sB[1][y8x] = tex2D(tex_B, idB_global, t+1);
		sB[2][y8x] = tex2D(tex_B, idB_global, t+2);
		sB[3][y8x] = tex2D(tex_B, idB_global, t+3);
		sB[4][y8x] = tex2D(tex_B, idB_global, t+4);
		sB[5][y8x] = tex2D(tex_B, idB_global, t+5);
		sB[6][y8x] = tex2D(tex_B, idB_global, t+6);
		sB[7][y8x] = tex2D(tex_B, idB_global, t+7);
