...
__global__ void matMul(const float *A, const float *B, float *C, int m, int k, int n)
{
	...
	for (int t = 0; t < k/TILE_WIDTH; ++t) {
		...
		for (int i = 0; i < TILE_WIDTH; i += WIDTH_PER_THREAD) {
			...
			b00 = sh_B00[i/WIDTH_PER_THREAD][tx/WIDTH_PER_THREAD];
			b01 = sh_B01[i/WIDTH_PER_THREAD][tx/WIDTH_PER_THREAD];
			b10 = sh_B10[i/WIDTH_PER_THREAD][tx/WIDTH_PER_THREAD];
			b11 = sh_B11[i/WIDTH_PER_THREAD][tx/WIDTH_PER_THREAD];
